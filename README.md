...
| 桌面端 | Tauri 2 | 原生性能，Rust 后端逻辑可复用到移动端 |
| 移动端 | Tauri 2 Mobile | 共享前端代码，Rust 原生层调用 Android API |
...
Release APK 需要签名才能安装到设备上。首次构建前需生成 keystore 并配置签名信息：

```bash
# 1. 生成 keystore (交互式，按提示设置密码和别名)
make android:keystore

# 2. 创建签名配置文件 gen/android/keystore.properties
#    keystore.properties 和 .jks 文件已加入 .gitignore，不会提交到仓库
```

`keystore.properties` 格式：

```properties
storeFile=purrchat-release.jks
storePassword=<密码>
keyAlias=purrchat
keyPassword=<密码>
```

`build.gradle.kts` 会自动读取该配置，无需手动修改 Gradle 文件。
