...
├── scripts/
│   ├── run-local-ci.sh     # 本地 CI/CD 脚本
│   └── merge-dev-to-test.sh # 合并脚本
...
项目采用三分支模型进行版本管理，使用 GitHub Actions 进行 CI/CD。
...
| 分支 | 用途 | CI/CD | 部署环境 |
|------|------|-------|----------|
| [`main`](main:1) | 生产环境 | GitHub Actions | purrchat.com |
| [`test`](main:1) | 预发布环境 | GitHub Actions | test.purrchat.com |
| [`dev`](main:1) | 开发环境 | 本地 CI/CD | 本地 |
...
项目包含完整的 GitHub Actions 配置 ([`.github/workflows/ci.yml`](.github/workflows/ci.yml:1))。
...
4. **deploy**: 部署到环境
   - 测试环境部署 ([`test`](main:1) 分支)
   - 生产环境部署 ([`main`](main:1) 分支)
...
| `TEST_SSH_PRIVATE_KEY` | 测试服务器 SSH 密钥 |
...
- **Push**: 推送到 `main`、`dev` 或 `test` 分支
- **Pull Request**: 针对 `main`、`dev` 或 `test` 分支的 PR
...
详细的 CI/CD 配置和使用说明请参考 [`CI_CD_GUIDE.md`](CI_CD_GUIDE.md:1)。
