...
- [x] Bot 引擎编排迁移至 TS 微服务 — Go botengine 标记为 deprecated (仅作 fallback)，触发评估和执行逻辑统一由 TS bot-engine 处理
- [x] Bot 调用记录 — Bot 所有者可跨会话查看 Bot 的调用历史（触发消息、回复内容、执行状态、耗时）
  - **隐私备忘**: 当前 beta 版允许 Bot 所有者查看所有会话中的触发消息内容（含其他用户的非公开消息）。在生产版本的公开 Bot 市场中，调用记录应仅展示元数据和 Bot 自身的回复内容，不应暴露其他用户的触发消息。正式版需做以下改造：
    1. 调用记录中 `trigger_message` 字段替换为脱敏摘要（如消息类型、长度，而非原文）
    2. Bot 所有者仅可查看自己参与的会话中的完整记录，其他会话仅显示元数据
    3. 新增 Bot 隐私设置，让 Bot 所有者选择是否公开调用记录给部署者查看
