...
原计划 BotApp 级域名白名单,引擎调用外部 URL 前校验 domain。**当前决定暂不实现**:白名单只能限制请求目标,无法保证白名单内端点本身安全(被攻陷的合法端点同样可窃取数据),而 `network:external` capability 声明 + diagnostics_consent 强制 granted + 安装时外发警示已覆盖知情同意的核心安全需求。`bots.allowed_endpoints` 字段已预留,如未来有合规需求可在此基础上补校验逻辑。
