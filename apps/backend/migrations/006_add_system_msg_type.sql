-- 006: 新增 'system' 消息类型 + 创建系统用户
-- 系统用户作为 system 消息和 Bot 消息的 sender_id（uuid.Nil）
-- 同时修复 sendBotReply 中 sender_id 为零值导致的外键约束失败

-- 1. 创建系统用户（uuid.Nil = 00000000-0000-0000-0000-000000000000）
INSERT INTO users (id, uid, username, password_hash, salt)
VALUES ('00000000-0000-0000-0000-000000000000', 0, 'System', 'unused', 'unused')
ON CONFLICT (id) DO NOTHING;

COMMENT ON TABLE users IS '用户表（含 System 系统用户用于系统消息）';

-- 2. 更新函数定义（影响新创建的会话消息表）
...
COMMENT ON FUNCTION create_conversation_message_table IS '为指定会话创建消息表（支持 system 消息类型）';

-- 3. 更新已有消息表的 CHECK 约束（影响已存在的会话消息表）
DO $$
DECLARE
    tbl RECORD;
BEGIN
