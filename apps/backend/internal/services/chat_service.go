...
// SetBotEngine 设置 Bot 引擎（在 main.go 中初始化后注入）
func (s *ChatService) SetBotEngine(engine *botengine.BotEngine) {
	s.botEngine = engine
}
...
func (s *ChatService) GetMessages(ctx context.Context, conversationIDStr string, limit, offset int) ([]*models.Message, error) {
	...
	for _, msg := range messages {
		if msg.BotID != nil && msg.BotName != nil && s.botRepo != nil {
...
func (s *ChatService) UpdateConversation(ctx context.Context, conversationIDStr, userID string, req *models.UpdateConversationRequest) (*models.Conversation, error) {
	conversationID, err := uuid.Parse(conversationIDStr)
	if err != nil {
		return nil, err
	}
...
func (s *ChatService) UpdateMemberRole(ctx context.Context, conversationIDStr, userID string, req *models.UpdateMemberRoleRequest) error {
	conversationID, err := uuid.Parse(conversationIDStr)
	if err != nil {
		return err
	}
...
func (s *ChatService) DeleteConversation(ctx context.Context, conversationIDStr, userID string) error {
	conversationID, err := uuid.Parse(conversationIDStr)
	if err != nil {
		return err
	}
	...
	if conversation.ConversationType != models.ConversationTypeGroup {
		return errors.New("can only delete group conversations")
	}
	...
	// 使用 conversation_repo 的方法删除（需要新增 Delete 方法）
	// 暂时直接执行 SQL
